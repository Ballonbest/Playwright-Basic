...
//msg
let loginmsgEmail:string = 'email must be a valid email'
...
test('Verify login page elements displayed correctly', async ({page}) => {
    await page.locator(loginEmailInput).fill("user.test@krupbeam.com")
    await page.locator(loginbtn).click()

    await expect(page.getByText(loginmsgEmail)).not.toBeVisible()
})

test.only('Should email containing non-English characters and invalid format',async ({page}) => {
    ...
    for(let x of data){
        ...
        await expect(page.getByText(loginmsgEmail)).toBeVisible()
    }
})